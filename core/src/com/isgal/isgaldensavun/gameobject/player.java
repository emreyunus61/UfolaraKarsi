...
//Player interface ile gameobject sınıfından türetildi ve playscreen sınıfında nesneyle üretildi.
...
public class player implements gameobject {
    ...
    public void render(SpriteBatch sb) {
        sb.begin();
        sb.draw(com.isgal.isgaldensavun.imageloader.imageLoader.playerReg,position.x,position.y,size.x,size.y);
        sb.end();
    }
    ...
    public void playerHareket(){
        ...
        if(position.x>-size.x/2||position.x>-10+size.x/2){
            if(position.x<Gdx.graphics.getWidth()-10||position.x<Gdx.graphics.getWidth()+10){
                if(position.x<=-size.x/2||position.x<-size.x/2.6){
                    position.add(10,0);
                    if (position.x<=-size.x/2||position.x<-size.x/2.6)
                        position.add(10,0);
                }
